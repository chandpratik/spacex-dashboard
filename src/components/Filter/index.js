...
function Filter() {
  ...
  return (
    <div className="flex-column w-52 h-8 mt-4 ml-auto">
      <div
        tabIndex={0}
        role="button"
      ...
      {open && (
        <ul className="py-2 border-2 rounded">
          {filterlist.map((filter) => (
            <li key={filter.id} className="py-1 hover:bg-gray-100">
              <button type="button" onClick={() => handleOnClick(filter.value)}>
